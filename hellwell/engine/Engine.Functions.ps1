...
function Add-LogRow([datetime]$start, [datetime]$end, [string]$name, [bool]$work, [bool]$sleep) {
  ...
  # Fix: for sleep sessions starting before 10am, assign to previous infernal day
  # (this is the end-of-night sleep, not an afternoon nap)
...
function Invoke-CommandLine([string]$cmdLine, $settings) {
  ...
  switch ($cmd) {
    ...
    "extend" {
      ...
      $n = 5
      $parsed = 0
      if ($arg -and [int]::TryParse($arg, [ref]$parsed)) { $n = $parsed }
      if ($n -lt 1) { $n = 1 }
      if ($s.Current.EndsAt) {
        try {
          $endAt = [datetime]::Parse($s.Current.EndsAt)
