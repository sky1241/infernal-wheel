...
test("empty pattern is not learned", not p.is_learned())
test("empty pattern scores 0 at all hours",
...
test("under-trained pattern is not learned", not p.is_learned())
test("under-trained pattern scores 0", p.score(9 * 60 + 30) == 0)
...
test("pattern is learned", p.is_learned())
...
test("9:30 scores high (>0.9)", score_9h > 0.9)
test("15:30 scores low (<0.1)", score_15h < 0.1)
test("is_high_smoking_minute true at 9:30", p.is_high_smoking_minute(9 * 60 + 30))
test("is_high_smoking_minute false at 15:30", not p.is_high_smoking_minute(15 * 60 + 30))
...
print(f"  score at 15:30 = {score_15h:.3f}")
test("9:30 is high", score_9h > 0.9)
test("15:30 is high", score_15h > 0.9)
test("12:30 is low (mid-morning gap)", score_12h < 0.2)
...
test("9:30 is the peak (score=1.0)",
     abs(score_9h30 - 1.0) < 0.01,
     f"score={score_9h30:.4f}")
test("9:00 and 10:00 (symmetric around peak) score identically",
     abs(score_9h00 - score_10h00) < 0.01,
     f"9:00={score_9h00:.4f} 10:00={score_10h00:.4f}")
test("9:15 and 9:45 (symmetric around peak) score identically",
     abs(score_9h15 - score_9h45) < 0.01,
     f"9:15={score_9h15:.4f} 9:45={score_9h45:.4f}")
test("closer to peak -> higher score (9:15 > 9:00)", score_9h15 > score_9h00)
test("no cliff at hour boundary (9:59 ~ 10:00)",
...
test("23:30 is the peak", score_23h30 > 0.9)
test("00:00 (midnight, 30 min from peak) scores similar to 23:00",
     abs(score_0h00 - score_23h00) < 0.1,
     f"23:00={score_23h00:.3f} 00:00={score_0h00:.3f}")
test("12:00 is far from peak, low score", score_12h00 < 0.1)
...
test("all scores are in [0, 1]", all_bounded)
